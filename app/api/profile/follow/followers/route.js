...
import { getFollowersForUser } from "@/app/lib/followService";
...
export const GET = async (req) => {
  ...
  if (isPrivateProfile(targetProfile) && !isProfileOwner(targetProfile, userId)) {
    return NextResponse.json(
      {
        message: "Followers are private for this profile.",
        privacyBlocked: true,
      },
      {
        status: 403,
  ...
  const usersList = await getFollowersForUser(targetProfile.userid);
